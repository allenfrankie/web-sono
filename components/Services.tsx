    ...
    <section id="expertise" className="py-24 bg-brand-black text-white relative">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 border-b border-gray-800 pb-8">
          <div>
            <h3 className="text-brand-yellow font-bold tracking-widest uppercase mb-2">{t.services.capabilities}</h3>
            <h2 className="text-4xl md:text-5xl font-display font-bold">{t.services.title}</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {SERVICES.map((service, index) => {
            ...
            // Get translated item if available, fallback to default
            const translatedItem = t.services.items[index] || { title: service.title, desc: service.description };
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-20 transition-opacity duration-700">
                  <img src={service.image} alt={translatedItem.title} className="w-full h-full object-cover grayscale" />
                </div>
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-3xl font-display font-bold mb-4">{translatedItem.title}</h3>
                  <p className="text-gray-400 text-lg leading-relaxed">{translatedItem.desc}</p>
                </div>

                <div className="relative z-10 mt-8 pt-8 border-t border-white/10 flex justify-between items-center">
                  <span className="text-sm font-bold uppercase tracking-widest text-gray-500 group-hover:text-white transition-colors">{t.services.learnMore}</span>
